...
### feat(sa): 서든어택 전적 검색 UX 고도화 및 분석 기능 강화
- **문제/증상**:
  - 표면적인 MMR 점수만으로는 유저의 실제 전투 능력(HSR)과 운영 능력 사이의 관계를 파악하기 어려움.
  - 내전 통계 문구가 딱딱하고 정형화되어 유저들에게 주는 재미 요소가 부족함.
  - 서든어택 페이지가 독립적으로 운영됨에도 불구하고 메인 홈으로 돌아가는 직관적인 경로가 부족함.
  - MVP 카드 내 K/D 표기가 비율(예: 1.5)로 되어 있어 서든 유저들에게 익숙한 킬레이트(%) 방식과 괴리가 있음.
- **변경**:
  - **MMR vs HSR 괴리 분석**: '승부사형(운영 우세)', '무력가형(피지컬 우세)' 분석 및 괴리가 클 경우 '팀운 부재'나 '무력 부족' 등의 솔직한 피드백 문구 추가.
  - **위트 있는 트롤링 문구 복구**: '내전 똥싼 판 💩', '기부천사', '역귀' 등 횟수에 따라 다이내믹하게 변하는 재미있는 성적 라벨 적용.
  - **서든전적 전용 홈 버튼**: 메인 사이트(본진) 레이아웃에 영향을 주지 않도록 검색 섹션 내부에 독립적인 `🏠 서든전적 홈` 링크 버튼 추가.
  - **K/D 표기 방식 표준화**: MVP 카드 내 K/D 지표를 서든어택 표준인 킬레이트 퍼센트(%) 형식으로 변경하여 직관성 향상.
  - **디자인 디테일 개선**: 매치 상세 정보 내 'CREW' 태그를 금색 테마로 통일하고 프리미엄 배경/테두리 효과 적용.
- **영향 범위**: `src/domains/games/sudden-attack/` 하위 UI 컴포넌트 전체.
- **검증**: `npm run build` 및 `npm run test:unit` (92개 테스트 전체 Pass) 확인.

### chore(actions): GitHub Actions 무료 시간 최적화 및 시스템 수정
- **문제/증상**:
  - GitHub Actions 무료 시간(2,000분/월) 한도 도달로 인해 자동화 작업 중단 위기.
