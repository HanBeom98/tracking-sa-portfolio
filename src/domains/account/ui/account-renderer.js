(function () {
  ...
  function setStatusText(element, messageKey, fallback, ok = false) {
    if (!element) return;
    const translate = window.AccountDomain && window.AccountDomain.viewmodel
      ? window.AccountDomain.viewmodel.t
      : ((_, text) => text);
  ...
  function renderAccountView(infoEl, actionsEl, user, viewModel) {
    const vm = window.AccountDomain && window.AccountDomain.viewmodel;
    const formatProvider = vm ? vm.formatProvider : (() => "-");
    const formatSubscription = vm ? vm.formatSubscription : (() => "-");
    const formatDate = vm ? vm.formatDate : (() => "-");
    const t = vm ? vm.t : ((_, text) => text);
