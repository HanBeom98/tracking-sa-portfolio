...
async function newsDeskPipeline(title, summary) {
    try {
        ...
        7. 용어 사전 링크 (SEO 필수): 본문 작성 중 다음 용어들이 자연스럽게 등장한다면, **반드시 HTML <a> 태그를 사용하여 용어 사전으로 링크를 거세요.** (글 전체에서 최대 2~3번만 사용)
           - 타겟 용어: LLM, RAG, NPU, 토큰(Token), 온디바이스(On-Device), 에이전틱(Agentic)
           - 링크 형식 예시: <a href="/glossary#LLM" class="glossary-link">LLM</a>`;
...
본문 중 'LLM', 'RAG', 'NPU' 등의 기술 용어가 있다면 자연스럽게 <a href="/glossary#용어"> 태그로 감싸주세요.

반드시 아래의 출력 형식을 엄격하게 지켜주세요:
[KO_START]
# (한국어 칼럼 제목)
(한국어 칼럼 본문 - 분석 중심, 1200자 이상, <a>태그 포함)
[KO_END]
...
위 원고를 검수하고, 오타를 수정하세요. 단, 기자가 작성한 HTML 태그(<a> 등)는 절대 건드리지 마세요.
