...
import { createFortuneCopy, resolveFortuneLanguage } from "../../src/domains/fortune/application/fortune-copy.js";
...
test("fortune language resolver normalizes to ko/en", () => {
  assert.equal(resolveFortuneLanguage("en"), "en");
  assert.equal(resolveFortuneLanguage("ko"), "ko");
  assert.equal(resolveFortuneLanguage("jp"), "ko");
});
